"""Spotified headless CLI - the GUI's engine with a terminal face.
...
def _resolve_out_dir(out_arg: str | None, cfg: dict) -> str:
    out = (
        ...
        or os.path.join(os.path.expanduser("~"), "Music", "Spotified")
    )
...
class _FolderLock:
    ...
    def acquire(self) -> str | None:
        ...
        if pid is not None and pid != os.getpid():
            return f"another spotified download (pid {pid}) is writing to this folder"
...
def cmd_download(args) -> int:
    ...
    if app.get_ffmpeg_path() is None:
        emitter.error(
            ...
            code="ffmpeg_missing",
            hint=(
                "the prebuilt spotified binaries bundle ffmpeg; source installs need it on PATH "
    ...
    try:
        ...
        emitter.error(f"download run failed: {exc}", code="run_failed", hint="run `spotified doctor`")
        return EXIT_FATAL
    finally:
        if lock is not None:
            lock.release()
...
def cmd_info(args) -> int:
    ...
    try:
        ...
        emitter.error(
            ...
            hint="check the url is public and reachable; `spotified doctor` tests upstream health",
        )
        return EXIT_FATAL
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="spotified",
        description=(
            "Spotified headless CLI: download Spotify playlists, albums, and tracks "
            "as tagged local audio files. Same engine and settings as the desktop app."
        ),
        epilog=(
            "examples:\n"
            '  spotified download "https://open.spotify.com/playlist/..."\n'
            '  spotified download "https://open.spotify.com/track/..." -o ~/Music -f flac --sample-rate 44100\n'
            '  spotified download "<url>" --json           # NDJSON progress events for scripts/agents\n'
            '  spotified info "<url>" --json               # metadata only, no download\n'
            "  spotified status                            # what's landed in the download folder\n"
            "  spotified config --set format=m4a           # persist a setting (shared with the GUI)\n"
            "  spotified doctor                            # self-check when downloads misbehave\n"
            "\nfull reference: https://github.com/sunnypatell/spotified-spotify-downloader/blob/main/docs/CLI.md"
        ),
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--version", "-V", action="version", version=f"spotified {app.__version__}")
    ...
    dl.add_argument(
        ...
        "-o",
        default=None,
        help=f"output folder (default: {cfg.get('download_path') or '~/Music/Spotified'})",
    )
    ...
    dl.add_argument("--no-lock", action="store_true", help="bypass the folder lock check")
