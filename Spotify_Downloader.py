#
"""
Spotified (Spotify Downloader)
Copyright (C) 2026 Shivam Kumar <jakemusicartist@gmail.com>
https://github.com/jakemusicartist-stack
...
<jakemusicartist@gmail.com> or open an issue in the repository.
"""
...
log = logging.getLogger("spotified")
...
def _install_crash_handlers() -> None:
    ...
    # crash.log sits next to spotified.log and stays open for the process lifetime.
    with contextlib.suppress(Exception):
        crash_path = os.path.join(os.path.dirname(log_file_path()), "crash.log")
        faulthandler.enable(open(crash_path, "a"))  # noqa: SIM115
    atexit.register(lambda: log.info("==== spotified session end ===="))
...
def get_ffmpeg_path():
    ...
    # Check project root directory (where Spotify_Downloader.py resides)
    root_dir = os.path.dirname(os.path.abspath(__file__))
    if os.path.exists(os.path.join(root_dir, ffmpeg_name)):
        return root_dir
...
MANIFEST_FILENAME = ".spotified-manifest.jsonl"
...
def _config_dir() -> str:
    ...
    path = os.path.join(base, "Spotified")
...
def _log_dir() -> str:
    ...
    files are where a user (or a support request) would expect them:
      windows -> %LOCALAPPDATA%\\Spotified\\logs
      macOS   -> ~/Library/Logs/Spotified
      linux   -> $XDG_STATE_HOME/spotified/logs (defaults to ~/.local/state)
    """
    if sys.platform == "win32":
        base = os.environ.get("LOCALAPPDATA") or os.environ.get("APPDATA", os.path.expanduser("~"))
        path = os.path.join(base, "Spotified", "logs")
    elif sys.platform == "darwin":
        path = os.path.join(os.path.expanduser("~"), "Library", "Logs", "Spotified")
    else:
        base = os.environ.get(
...
def log_file_path() -> str:
    ...
    return os.path.join(_log_dir(), "spotified.log")
...
def setup_logging() -> str:
    ...
    """
    if any(getattr(h, "_spotified", False) for h in log.handlers):
    ...
    handler._spotified = True  # tag so we don't double-attach on re-call
    ...
    # SPOTIFIED_DEBUG=1 to get the full per-track + yt-dlp trail for hard cases.
    level = logging.DEBUG if os.environ.get("SPOTIFIED_DEBUG") else logging.INFO
    log.setLevel(level)
    ...
    try:
        ...
        ytdlp_ver = "?"
    log.info("==== spotified session start ====")
    log.info(
...
GITHUB_REPO = "sunnypatell/spotified-spotify-downloader"
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent, config: dict):
        ...
        self.setWindowTitle("Spotified Settings")
        self.setModal(True)
    ...
    def _choose_folder(self):
        ...
        if folder:
            # Only append "Spotified" when the user picked a non-Spotified folder,
            # otherwise re-selecting the existing destination creates nested
            # Spotified/Spotified/... paths.
            chosen = (
                folder
                if os.path.basename(folder.rstrip(os.sep)) == "Spotified"
                else os.path.join(folder, "Spotified")
            )
...
class UpdateNotifier(QDialog):
    ...
    def __init__(self, parent, current: str, latest: str, url: str):
        ...
        msg = QLabel(
            "A newer version of Spotified is available. Download it from the "
...
class StarPromptNotifier(QDialog):
    ...
    def __init__(self, parent):
        ...
        self.setWindowTitle("Enjoying Spotified?")
        self.setAttribute(Qt.WidgetAttribute.WA_TranslucentBackground)
        self.setWindowFlags(Qt.WindowType.Dialog | Qt.WindowType.FramelessWindowHint)
        self.setModal(True)
        self.setFont(QFont("Arial", 10))
        ...
        name = QLabel("Enjoying Spotified?")
        nfont = QFont("Arial", 22, QFont.Weight.Bold)
        name.setFont(nfont)
        name.setStyleSheet("color: #FFFFFF;")
        # large bold glyphs exceed QLabel's tight default box; reserve full height
        ...
        msg = QLabel(
            ...
            "find Spotified, and it takes five seconds. This asks once and never "
            "again."
        )
        msg.setWordWrap(True)
        msg.setFont(QFont("Arial", 10))
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def _get_default_download_path(self):
        ...
        music_folder = os.path.join(home, "Music", "Spotified")
        ...
        if sys.platform == "win32":
            try:
                ...
                music_folder = os.path.join(winreg.QueryValueEx(key, "My Music")[0], "Spotified")
                winreg.CloseKey(key)
            except Exception:
                music_folder = os.path.join(home, "Music", "Spotified")
    ...
    def _ensure_download_path(self):
        ...
        try:
            ...
            # Test write access
            test_file = os.path.join(self.download_path, ".spotified_test")
    ...
    def _prompt_download_location(self):
        ...
        if folder:
            # Keep downloads contained in a "Spotified" subfolder, but avoid
            # creating nested Spotified/Spotified/... paths when the user picked
            # a folder that's already named Spotified.
            if os.path.basename(folder.rstrip(os.sep)) == "Spotified":
                self.download_path = folder
            else:
                self.download_path = os.path.join(folder, "Spotified")
...
if __name__ == "__main__":
    ...
    # routes to spotified_cli; anything else - including a bare double-click -
    ...
    if _arg1 in _cli_commands or _arg1 in ("--version", "-V", "--help", "-h"):
        ...
        import spotified_cli

        sys.exit(spotified_cli.main(sys.argv[1:]))
    if _arg1.isascii() and _arg1.isalpha():
        ...
        print(
            f"spotified: unknown command '{_arg1}'{hint}\nrun 'spotified --help' for usage",
            file=sys.stderr,
        )
        sys.exit(2)
